#!/usr/bin/env bash
#
# Train MR-LFADS on 4 real-data sessions sequentially.
# Designed to survive terminal disconnection (run via nohup).
#
# Usage:
#   nohup bash scripts/train_all_sessions.sh > logs/train_all_sessions.log 2>&1 &
#
set -euo pipefail

cd /home/mr_lfads_repro

SESSIONS=("20200929" "20201001" "20231025" "20231103")
CONFIG="configs/rct_fef_lip_sc.json"

mkdir -p logs

echo "========================================"
echo "Starting batch training: ${#SESSIONS[@]} sessions"
echo "Time: $(date)"
echo "========================================"
